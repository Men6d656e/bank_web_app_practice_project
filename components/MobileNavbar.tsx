...
function MobileNavbar({ user }: MobileNavProps) {
    ...
    return (
        <section className="w-full max-w-[264px]">
            <Sheet>
                <SheetTrigger>
                    <Image src="/icons/hamburger.svg" className="cursor-pointer" alt="hamburger" width={30} height={30} />
                </SheetTrigger>
                                    ...
                                    return (
                                        <SheetClose asChild key={link.route}>
                                            ...
                                            <Link href={link.route} key={link.label} className={cn('mobilenav-sheet_close w-full ', { 'bg-bank-gradient': isActive })}>
                                                <Image width={20} height={20} className={cn({ 'brightness-[3] invert-0': isActive })} src={link.imgURL}  alt={link.label} />
